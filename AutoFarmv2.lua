...
-- ══════════════════════════════
--   AUTO ATTACK - Tool Based
-- ══════════════════════════════
-- Lấy Tool đang cầm trong tay nhân vật
...
-- Tìm RemoteEvent dùng để đánh trong Tool (thường tên "Attack", "DamageEvent", v.v.)
local function findAttackRemote(tool)
...
-- Đánh bằng Tool: ưu tiên FireServer remote, fallback activate tool script
local function doToolAttack(targetHRP)
    local tool = getEquippedTool()
    if not tool then return end

    -- Cập nhật Hit/Target của Mouse để tool nhận đúng mục tiêu
    -- (không di chuột thật – chỉ set target cho tool)
    local mouse = LocalPlayer:GetMouse()
    -- Một số game đọc Mouse.Hit để xác định hướng đánh
    -- Ta dùng CFrame giả để ép hướng đúng vào NPC
    local fakeHit = CFrame.new(targetHRP.Position)

    -- 1) Thử FireServer remote event (phổ biến nhất)
    local remote = findAttackRemote(tool)
    if remote then
        pcall(function()
            remote:FireServer(fakeHit, targetHRP.Parent)
        end)
        return
    end

    -- 2) Fallback: kích hoạt LocalScript bên trong Tool
    -- bằng cách tạm thời fire event Activated của tool
    ...
    -- 3) Fallback cuối: gọi thẳng hàm Activate (không dùng chuột)
    pcall(function()
        tool:Activate()
    end)
end
